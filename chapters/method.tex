...
\textcolor{red}{
\begin{itemize}
    \item briefly describe the general concept / framework
    \begin{itemize}
...
\begin{figure}
    ...
    \def\blockheight{4cm}
    \def\blockwidth{4.5cm}
    \def\smallblockheight{0.7cm}
    \def\smallblockwidth{4cm}
    \def\smallyshift{0.8cm}
    \def\labelyshift{-0.8cm}
    \def\xshiftrightblocks{2cm}
    
        % Blocks
        % Trajectory System
        \node[draw, fill=mumblue, fill opacity=0.1, semithick, rectangle, rounded corners=0.2cm, densely dashed, minimum width=\blockwidth, minimum height=\blockheight, align=center, label={[label distance=\labelyshift]90:Trajectory Module}] (trajectoryblock) at (0,0) {};
        \node[draw, fill=white, semithick, rectangle, minimum width=\smallblockwidth, minimum height=\smallblockheight, align=center] (generation) at (0,\smallyshift) {Generation};
        \node[draw, fill=white, semithick, rectangle, minimum width=\smallblockwidth, minimum height=\smallblockheight, align=center, below= of generation, yshift=\smallyshift] (tracking) {Tracking controller};
        \node[draw, fill=white, semithick, rectangle, minimum width=\smallblockwidth, minimum height=\smallblockheight, align=center, below= of tracking, yshift=\smallyshift] (feasibility) {Feasibility tests};
        \node[draw, fill=white, semithick, rectangle, minimum width=\smallblockwidth, minimum height=\smallblockheight, align=center, below= of feasibility, yshift=\smallyshift] (obstacles) {Obstacle avoidance};

        % Low Level Controller
        \node[draw, fill=mumpurple, fill opacity=0.1, semithick, rectangle, rounded corners=0.2cm, densely dashed, minimum width=\blockwidth, minimum height=\blockheight, align=center, label={[label distance=\labelyshift]90:Low Level Controller}, right= of trajectoryblock, xshift=\xshiftrightblocks] (lowlevelcontrolblock) {};
        \node[draw, fill=white, semithick, rectangle, minimum width=\smallblockwidth, minimum height=\smallblockheight, align=center, below= of lowlevelcontrolblock, yshift=3cm] (ratecontroller) {Rate controller};
        \node[draw, fill=white, semithick, rectangle, minimum width=\smallblockwidth, minimum height=\smallblockheight, align=center, below= of ratecontroller, yshift=\smallyshift] (thrust) {Thrust controller};



        % High level Planner
        \node[draw, fill=lightgray, semithick, rectangle, rounded corners=0.2cm, densely dashed, minimum width=\blockwidth, minimum height=\blockheight-1cm, align=center, above= of trajectoryblock, label={[label distance=\labelyshift]90:High Level Planner}] (plannerblock) {\includegraphics[width=.25\textwidth]{images/03/high_level_planner.png}};  % high_level_planner3.png ist scharf aber kein so geeigneter Zeitpunkt
...
% In order to describe the 6\,DOF motion of an underwater vehicle we define two main geographic reference frames.

% \paragraph{World-fixed Frame $\wfr$}
% Following the standard notation for marine crafts introduced by Fossen~\cite{Fossen2011} it is common to define a \textit{North-East-Down}\,(NED) coordinate system  with its origin $\p{O}{W}$  relative to the earth's reference ellipsoid (World Geodetic System, 1984). 
% Its axes are defined through the unit vectors $\bm{x}_\nfr$, $\bm{y}_\nfr$, and $\bm{z}_\nfr$ pointing towards true north, east, and in downward direction, respectively.
% However, given our focus application field of confined environments, it is often convenient to use a more flexible definition of the inertial reference frame to which we refer to as world frame $\wfr$.
% We define this world frame by the unit vectors $\bm{x}_\wfr$, $\bm{y}_\wfr$, and $\bm{z}_\wfr$ and the origin $\p{O}{W}$.
...
% Hence, a convenient approach is to define the $x_\wfr$- and $y_\wfr$- axis along the volume dimensions, \eg along the walls of a rectangular tank while the $z_\wfr$- axis points in downward direction.
% For the sake of simplicity, we use the world frame throughout the remainder of this dissertation.
% During outdoor experiments, however, our world frame definition coincides with the NED convention.

% \paragraph{Body-fixed Frame $\bfr$}
% Moreover, we define a moving \textit{body} frame $\bfr$ which is fixed to the vehicle of interest, \ie the HippoCampus \muAUV{}.
% By definition the body frame's origin $\p{O}{B}$ coincidences with the vehicles center of gravity\,(CG).
% We choose its axes $x_\bfr$, $y_\bfr$, and $z_\bfr$ such that they align with the vehicles \textit{principal axes of inertia}. 
% For the design of the HippoCampus \muAUV{} we define
% \begin{itemize}
%     \item $x_\bfr$: longitudinal axis (directed from aft to fore)
%     \item $y_\bfr$: transversal axis (directed to starboard)
%     \item $z_\bfr$: normal axis (directed from top to bottom)
% \end{itemize}
% as depicted in Figure\,\ref{fig:def_reference_frames}.
...
Recall the coordinate frame $\mathcal{C}$ from \Cref{sec:differential-flatness}
\begin{equation}
	\eyinterinworld =
	\begin{bmatrix}
...
The control architecture presendet in \Cref{sec:control} is implemented in a modular design in \ac{ros2}. While being the successor of \ac{ros}, \ac{ros2} is developed independently from \ac{ros}.
...
Based on limitations identified in \Cref{sec:sw_limitatations}....
